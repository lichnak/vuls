...
func getS3() *s3.S3 {
	Config := &aws.Config{
		Region: aws.String(c.Conf.Report.AwsRegion),
		Credentials: credentials.NewChainCredentials([]credentials.Provider{
			&credentials.EnvProvider{},
			&credentials.SharedCredentialsProvider{Filename: "", Profile: c.Conf.Report.AwsProfile},
...
func CheckIfBucketExists() error {
	...
	if err != nil {
		return fmt.Errorf(
			...
			err, c.Conf.Report.AwsProfile, c.Conf.Report.AwsRegion)
	}

	found := false
	for _, bucket := range result.Buckets {
		if *bucket.Name == c.Conf.Report.S3Bucket {
			found = true
			break
		}
	}
	if !found {
		return fmt.Errorf(
			...
			c.Conf.Report.AwsProfile, c.Conf.Report.AwsRegion, c.Conf.Report.S3Bucket)
	}
	return nil
}
...
func putObject(svc *s3.S3, k string, b []byte) error {
	...
	putObjectInput := &s3.PutObjectInput{
		Bucket: aws.String(c.Conf.Report.S3Bucket),
		Key:    aws.String(path.Join(c.Conf.Report.S3ResultsDir, k)),
		Body:   bytes.NewReader(b),
	}

	if c.Conf.Report.S3ServerSideEncryption != "" {
		putObjectInput.ServerSideEncryption = aws.String(c.Conf.Report.S3ServerSideEncryption)
	}

	if _, err := svc.PutObject(putObjectInput); err != nil {
		return fmt.Errorf("Failed to upload data to %s/%s, %s",
			c.Conf.Report.S3Bucket, k, err)
	}
	return nil
}
